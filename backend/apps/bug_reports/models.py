...
class BugReport(TimestampedModel):
    ...
    reporter_email = models.EmailField(
        help_text="Email address of the bug reporter"
    )
    
    # Bug details
    title = models.CharField(
        max_length=200,
        help_text="Brief summary of the bug"
    )
    ...
    application_state = models.JSONField(
        default=dict,
        help_text="Application state when bug occurred"
    )
    ...
    github_issue_url = models.URLField(
        blank=True,
        help_text="URL of the created GitHub issue"
    )
    ...
    labels = models.JSONField(
        default=list,
        help_text="GitHub labels applied to the issue"
    )
