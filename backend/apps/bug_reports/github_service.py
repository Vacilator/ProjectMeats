...
class GitHubIssueService:
        ...
        try:
            ...
            if response.status_code == 201:
                ...
                return {
                    ...
                    'assigned_to_copilot': self.copilot_username in [a['login'] for a in issue_data.get('assignees', [])],
    ...
    def _format_issue_body(self, bug_report) -> str:
        ...
        body_parts.append(f"**Reported by:** {bug_report.reporter.get_full_name() or bug_report.reporter.username}")
        body_parts.append(f"**Email:** {bug_report.reporter_email}")
        body_parts.append(f"**Priority:** {bug_report.get_priority_display()}")
        body_parts.append(f"**Reported on:** {bug_report.created_on.strftime('%Y-%m-%d %H:%M:%S UTC')}")
        body_parts.append("")
        ...
        # Footer
        body_parts.append("---")
        body_parts.append("*This issue was automatically created from a bug report in ProjectMeats.*")
