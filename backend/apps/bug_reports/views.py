...
class BugReportViewSet(viewsets.ModelViewSet):
    ...
    def create(self, request, *args, **kwargs):
        ...
        try:
            ...
            response_serializer = BugReportSerializer(bug_report, context={'request': request})
    ...
    def _create_github_issue(self, bug_report):
        ...
        try:
            ...
            if result.get('success'):
                ...
                bug_report.assigned_to_copilot = result.get('assigned_to_copilot', False)
                bug_report.labels = result.get('labels', [])
                bug_report.status = BugReportStatus.SUBMITTED
                bug_report.save()
                
                logger.info(f"Created GitHub issue #{result['issue_number']} for bug report #{bug_report.id}")
                
            else:
                # Mark as failed and store error message
                bug_report.status = BugReportStatus.FAILED
                bug_report.error_message = result.get('error', 'Unknown error creating GitHub issue')
                bug_report.save()
                
                logger.error(f"Failed to create GitHub issue for bug report #{bug_report.id}: {result.get('error')}")
            ...
            logger.error(f"Exception creating GitHub issue for bug report #{bug_report.id}: {str(e)}")
    ...
    def retry_github_creation(self, request, pk=None):
        ...
        if bug_report.status != BugReportStatus.FAILED:
            return Response(
                {
                    'success': False,
                    'error': 'Can only retry failed bug reports'
                },
    ...
    def stats(self, request):
        ...
        stats = {
            ...
            'by_status': {},
            'by_priority': {},
            'submitted_to_github': queryset.filter(status=BugReportStatus.SUBMITTED).count(),
            'assigned_to_copilot': queryset.filter(assigned_to_copilot=True).count(),
            'failed_submissions': queryset.filter(status=BugReportStatus.FAILED).count(),
        }
    ...
    def user_reports(self, request):
        ...
        """
        user_reports = BugReport.objects.filter(reporter=request.user).order_by('-created_on')
