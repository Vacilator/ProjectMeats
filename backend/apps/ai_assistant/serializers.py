...
class ChatSessionListSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = ChatSession
        fields = [
            'id', 'title', 'session_status', 'status',
            'owner', 'message_count', 'has_documents',
            'last_activity', 'created_on'
        ]
        read_only_fields = [
            'id', 'owner', 'message_count', 'has_documents',
            'last_activity', 'created_on'
        ]
...
class ChatSessionDetailSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = [
            'id', 'title', 'session_status', 'status',
            'context_data', 'owner', 'message_count', 'has_documents',
            'last_activity', 'created_on', 'modified_on',
...
class UploadedDocumentSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = [
            ...
            'file_size_mb', 'file_type', 'document_type', 'confidence_score',
            'processing_status', 'is_processed', 'extracted_text',
            'extracted_data', 'processing_metadata', 'processing_error',
            'created_entities', 'owner', 'status', 'created_on'
        ]
        read_only_fields = [
            'id', 'file_size', 'file_size_mb', 'file_type', 'confidence_score',
            'processing_status', 'is_processed', 'extracted_text',
            'extracted_data', 'processing_metadata', 'processing_error',
            'created_entities', 'owner', 'created_on'
        ]
...
class ChatMessageSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = [
            ...
            'is_processed', 'processing_error', 'uploaded_document',
            'owner', 'created_on', 'modified_on', 'created_by', 'modified_by'
        ]
        read_only_fields = [
            'id', 'owner', 'is_processed', 'processing_error',
...
class ProcessingTaskSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = [
            'id', 'task_type', 'document', 'session', 'status',
            'progress_percentage', 'input_data', 'output_data',
            'error_details', 'owner', 'created_on', 'started_at',
            'completed_at', 'duration'
        ]
        read_only_fields = [
            'id', 'document', 'session', 'status', 'progress_percentage',
            'output_data', 'error_details', 'owner', 'created_on',
...
class ChatBotRequestSerializer(serializers.Serializer):
    ...
    message = serializers.CharField(
        max_length=5000,
        help_text="User message to send to the AI assistant"
    )
...
class ChatBotResponseSerializer(serializers.Serializer):
    ...
    session_id = serializers.UUIDField(
        help_text="Session ID for the conversation"
    )
...
class DocumentProcessingRequestSerializer(serializers.Serializer):
    ...
    session_id = serializers.UUIDField(
        required=False,
        help_text="Optional session ID to associate with processing"
    )
    processing_options = serializers.JSONField(
        required=False,
        default=dict,
        help_text="Optional processing configuration"
    )
...
class DocumentProcessingResponseSerializer(serializers.Serializer):
    ...
    task_id = serializers.UUIDField(
        help_text="ID of the processing task"
    )
    document_id = serializers.UUIDField(
        help_text="ID of the document being processed"
    )
