...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        self.stdout.write(
            self.style.SUCCESS('🔍 Django Deployment Diagnostics\n')
        )
    ...
    def validate_secret_key(self, key):
        ...
        if issues:
            self.stdout.write(
                self.style.ERROR('❌ Secret key validation failed:')
            )
            for issue in issues:
            ...
            self.stdout.write(
                self.style.SUCCESS('✅ Secret key is deployment-safe\n')
            )
        ...
        for setting_name, description in checks:
            try:
                ...
                if value:
                    ...
                    self.stdout.write(
                        self.style.WARNING(f'⚠️  {setting_name}: Not set - {description}')
                    )
            except Exception as e:
                self.stdout.write(
    ...
    def run_full_diagnostics(self):
        ...
        # Final summary
        self.stdout.write(
            self.style.SUCCESS('📋 Diagnostics complete!')
        )
        self.stdout.write(
