...
from .serializers import PurchaseOrderListSerializer, PurchaseOrderDetailSerializer, PurchaseOrderCreateSerializer
...
@extend_schema_view(
    ...
    update=extend_schema(
        summary="Update Purchase Order", description="Update an existing purchase order.", tags=["Purchase Orders"]
    ),
...
class PurchaseOrderViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        if self.request.query_params.get("fulfilled") == "true":
            ...
            queryset = queryset.filter(
                fulfillment_date__isnull=False, fulfillment_date__date__lte=timezone.now().date()
            )
        elif self.request.query_params.get("pending") == "true":
            from django.utils import timezone

            queryset = queryset.filter(fulfillment_date__isnull=False, fulfillment_date__date__gt=timezone.now().date())
        ...
        if self.request.query_params.get("has_documents") == "true":
            queryset = queryset.exclude(
                ...
                supplier_documents="",
            ).exclude(customer_documents__name="", supplier_documents__name="")
    ...
    def _get_default_user(self):
        ...
        user, created = User.objects.get_or_create(
            username="system",
            defaults={"email": "system@projectmeats.com", "first_name": "System", "last_name": "User"},
        )
        return user
