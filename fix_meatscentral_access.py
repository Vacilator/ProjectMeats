...
class MeatsCentralFixer:
    ...
    def local_diagnostics(self):
        ...
        try:
            ...
            if resolved_ip == self.server_ip:
                ...
                self.print_error(
                    f"{self.domain} resolves to {resolved_ip}, but should be {self.server_ip}"
                )
                self.print_info("DNS is not pointing to your server!")
                self.print_info(
                    "You need to update your domain's A record to point to 167.99.155.140"
                )
                return False
        ...
        for port, service in [(80, "HTTP"), (443, "HTTPS"), (22, "SSH")]:
            try:
                ...
                if result == 0:
                    ...
                    if port == 80:
                        self.print_error(
                            f"Port {port} ({service}) is NOT accessible - This is the problem!"
                        )
                    elif port == 443:
                        self.print_info(
                            f"Port {port} ({service}) is not accessible (normal without SSL)"
                        )
                    else:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                self.print_warning(
                    f"Direct IP access returned status {response.status_code}"
                )
        except requests.exceptions.ConnectionError:
    ...
    location /media/ {{
        ...
        exit_code, stdout, stderr = self.run_command(
            f"ln -sf /etc/nginx/sites-available/{self.domain} /etc/nginx/sites-enabled/"
        )
        if exit_code == 0:
        ...
        if exit_code != 0:
            self.print_warning(
                "ProjectMeats service not found, creating basic service configuration"
            )
...
WantedBy=multi-user.target"""
        ...
        if exit_code == 0 and "healthy" in stdout:
            ...
            self.print_warning(
                "Health check still not working - checking service status"
            )
    ...
    def print_recommendations(self):
        ...
        print(
            f"{Colors.BOLD}To fix meatscentral.com access, follow these steps:{Colors.END}\n"
        )
...
def main():
    ...
    parser.add_argument(
        "--local-check", action="store_true", help="Run diagnostics from local machine"
    )
    parser.add_argument(
        "--server-check", action="store_true", help="Run diagnostics on server"
    )
    parser.add_argument(
        "--auto-fix",
        action="store_true",
        help="Attempt to auto-fix issues (run on server)",
    )
    ...
    if args.local_check:
        ...
        print("Usage:")
        print(
            "  python fix_meatscentral_access.py --local-check    (run from your computer)"
        )
        print("  python fix_meatscentral_access.py --server-check   (run on server)")
        print(
            "  python fix_meatscentral_access.py --auto-fix       (run on server to fix)"
        )
