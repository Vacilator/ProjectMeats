...
class AIAssistantSetup:
    ...
    def prompt_input(
        ...
        while True:
            ...
            if default:
                self.log(
                    f"  (Press Enter for default: {default})", "INFO", Colors.YELLOW
                )
    ...
    def prompt_choice(self, prompt: str, choices: List[str], default: int = 0) -> str:
        ...
        while True:
            ...
            try:
                choice_input = input(
                    f"  Enter choice (1-{len(choices)}, default: {default + 1}): "
                ).strip()
                ...
                if 0 <= choice_index < len(choices):
                    ...
                    self.log(
                        f"Please enter a number between 1 and {len(choices)}", "ERROR"
                    )
            except ValueError:
    ...
    def welcome_message(self):
        ...
        print(
            f"  {Colors.GREEN}✓{Colors.END} AI provider configuration (OpenAI, Anthropic, etc.)"
        )
    ...
    def configure_django_backend(self):
        ...
        # Debug mode
        debug_mode = self.prompt_boolean(
            "Enable DEBUG mode? (recommended for development)", True
        )
        ...
        if debug_mode:
            ...
            allowed_hosts = self.prompt_input(
                "Enter allowed hosts (comma-separated)", "your-domain.com,localhost"
            )
        ...
        if "SQLite" in db_choice:
            ...
            pg_password = self.prompt_input(
                "Database password", required=True, secret=True
            )
    ...
    def configure_ai_providers(self):
        ...
        primary_provider = self.prompt_choice(
            "Select your primary AI provider:", providers, 0
        )
    ...
    def _configure_openai(self):
        ...
        # Max tokens
        max_tokens = self.prompt_input("Enter max tokens (default: 2000)", "2000")
    ...
    def _configure_azure_openai(self):
        ...
        deployment = self.prompt_input("Enter your deployment name", "gpt-35-turbo")
        self.env_vars["AZURE_OPENAI_DEPLOYMENT"] = deployment

        api_version = self.prompt_input("Enter API version", "2024-02-01")
    ...
    def _configure_mock_provider(self):
        ...
        self.config["ai_provider"] = {
            ...
            "configuration": {"response_delay": 0.5, "enable_detailed_responses": True},
            "is_default": True,
        }
    ...
    def _configure_backup_providers(self, primary_provider: str):
        ...
        self.log(
            "You can add backup providers later through the admin interface", "INFO"
        )
    ...
    def configure_email_and_notifications(self):
        ...
        if "Console" in email_choice:
            ...
            self.env_vars["EMAIL_HOST"] = self.prompt_input(
            ...
            self.env_vars["EMAIL_HOST_USER"] = self.prompt_input(
                "Email username", validation_func=self.validate_email
            )
            self.env_vars["EMAIL_HOST_PASSWORD"] = self.prompt_input(
                "Email password (or app password)", secret=True
            )
        else:
            self.env_vars["EMAIL_BACKEND"] = (
...
"""
    ...
    def install_dependencies(self):
        ...
        if self.frontend_dir.exists():
            ...
            try:
                ...
                self.log(
                    "You may need to run 'npm install' manually in the "
                    "frontend directory",
                    "WARNING",
                )
                ...
                self.log(
                    ...
                    "manually in the frontend directory",
                    "WARNING",
                )
    ...
    def initialize_database(self):
        ...
        init_db = self.prompt_boolean(
            "Initialize database and create admin user?", True
        )
    ...
    def _create_admin_user(self):
        ...
        email = self.prompt_input(
            "Admin email", "admin@projectmeats.com", validation_func=self.validate_email
        )
...
"""
    ...
    def display_completion_message(self):
        ...
        print(
            f"     {Colors.YELLOW}cd backend && python manage.py runserver{Colors.END}"
        )
        print()
        ...
        print(
            f"     {Colors.YELLOW}Backend API: http://localhost:8000/api/v1{Colors.END}"
        )
        print(
            f"     {Colors.YELLOW}Admin Panel: http://localhost:8000/admin{Colors.END}"
        )
        print()

        if (
            "ai_provider" in self.config
            and self.config["ai_provider"]["provider"] != "local"
        ):
            self.log("AI Assistant Features:", "INFO")
            print(
                f"  {Colors.GREEN}•{Colors.END} Chat interface with intelligent responses"
            )
            print(f"  {Colors.GREEN}•{Colors.END} Document upload and processing")
            print(
                f"  {Colors.GREEN}•{Colors.END} Entity extraction and data integration"
            )
            ...
            print(
                f"  {Colors.YELLOW}•{Colors.END} Configure a real AI provider for full functionality"
            )

        print()
        self.log("Documentation:", "INFO")
        print(
            f"  {Colors.CYAN}•{Colors.END} API Documentation: http://localhost:8000/api/docs/"
        )
        print(
            f"  {Colors.CYAN}•{Colors.END} Setup Guide: docs/setup-and-development.md"
        )
        print(
            f"  {Colors.CYAN}•{Colors.END} AI Assistant Guide: docs/ai_assistant_guide.md"
        )
        print()
