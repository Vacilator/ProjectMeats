...
""".strip()
        ...
        try:
            ...
            self.log("Ensuring clean state for admin script creation...")
            ...
            self.log("Searching for files with incorrect imports...")
            try:
            ...
            # Create admin script with correct imports
...
"""
            ...
            # Create and execute the script with validation
            self.log("Creating admin script...")
            ...
            try:
                ...
                if "from apps.core.models import UserProfile" not in verification_content:
                    raise Exception("Admin script missing correct import")
                
                if "apps.user_profiles" in verification_content:
                    raise Exception("Admin script contains incorrect user_profiles import")
                
                self.log("Admin script created and verified successfully", 'SUCCESS')
                
                # Execute using shell redirection (primary method)
                ...
                try:
                    ...
                    self.log(f"Shell redirection failed: {shell_error}", 'WARNING')
                    # Fallback: Direct Python execution
                    try:
                        self.log("Trying direct Python execution...")
                        self.run_command(f"cd {backend_dir} && ./venv/bin/python create_admin.py")
                        self.log("Direct execution succeeded", 'SUCCESS')
                    except Exception as direct_error:
                        self.log(f"Direct execution failed: {direct_error}", 'WARNING')
                        # Final fallback: Inline execution
                        try:
                            self.log("Trying inline execution...")
                            import shlex
                            ...
                            self.log("Inline execution succeeded", 'SUCCESS')
